...
library('tidyverse')
library('ggrepel')
setwd('C:/Users/am4613/OneDrive - Imperial College London/Simona_project/figure_4')
...
bmp_de <- results(grouped_ds, contrast=c('group','KOcontrol_endo','floxcontrol_endo'))
...
cc_de <- results(grouped_ds, contrast=c('group','floxbmp','floxcontrol_bmp'))
pdf('bmp_KO_comparison.pdf')
...
write.table(up_cc_down_bmp, 'up_bmp_down_p53.csv', sep = ',')

down_cc_up_bmp <- all_common[which(all_common$log2FoldChange.x < 0 & all_common$log2FoldChange.y > 0 ),]
write.table(down_cc_up_bmp, 'down_bmp_up_p53.csv', sep = ',')
...
comparison_and_enrichment(bmp_de, cc_de, labels = c('p53_KO','bmp'), plot_title = 'bmp_vs_p53.pdf', k = 10)

ko_bmp_de <- results(grouped_ds, contrast=c('group','KObmp','floxbmp'))
write.table(ko_bmp_de, 'ko_bmp_vs_flox_bmp.csv', sep = ',')
