...
write.table(bmp_de, 'p53KO_de.csv', sep = ',')
pval_threshold <- 0.05
fc_threshold <- 0
...
##KEGG
pdf(file = 'KO_DE_enrichment.pdf', width = 25, height = 25)
...
pdf('coculture_KO_DE.pdf', width = 7, height = 7)
...
dev.off()
ggsave(p2, filename = 'bubble_plot_KO.pdf', scale = 2)

comparison_and_enrichment(bmp_de, cc_de, labels = c('KO','coculture'), plot_title = 'KO_vs_coculture.pdf', k = 12)
